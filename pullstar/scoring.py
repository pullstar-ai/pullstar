"""
pullstar.scoring — Deterministic scoring engine for PullStar 1-on-1

Usage (CLI, via the thin wrapper):
    python scripts/score.py --login jsmith
    python scripts/score.py --login jsmith --input-dir .pullstar --output-dir .pullstar

All 5 dimensions. No AI calls. Pure computation. Max score = 100.

This module is import-safe: importing it runs no I/O and parses no arguments.
The scoring functions (``score_velocity`` … ``score_consistency``) are the
reusable engine seam; ``main`` implements the file-based CLI.
"""
